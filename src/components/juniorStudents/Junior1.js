...
const Junior1 = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/junior1')
      .then(response => {
        setStudents(response.data);
        setLoading(false);
      })
      .catch(error => {
        console.error('Error fetching junior 1 students:', error);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Junior 1 Class</h2>
      {loading ? (
