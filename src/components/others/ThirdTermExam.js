...
const ThirdTermExam = () => {
  ...
  const fetchExamRecords = (studentId) => {
    ...
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/third-term/${encodeURIComponent(studentId)}`)
      .then((response) => {
...
const handleSubmit = (e) => {
  ...
  axios
    .post('http://localhost:5000/api/add-third-term-result', {
    ...
    .catch((error) => {
      setMessage('Error adding exam and ca must not be greater than 100.');
      setLoading(false);
  ...
  const deleteExamRecord = (id) => { 
    ...
    axios
      .delete(`http://localhost:5000/api/delete-first/${id}`)
      .then((response) => {
  ...
  const handleUpdateSubmit = () => {
    ...
    axios
    .put(`http://localhost:5000/api/update-third/${recordToEdit.id}`, updatedData)
    .then((response) => {
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Add Third Term Examination</h2>
      {message && (
        <p
          style={{
            ...styles.message,
