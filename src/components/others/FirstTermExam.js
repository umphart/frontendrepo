...
useEffect(() => {
  ...
  const fetchExamRecords = (studentId) => {
    ...
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/first-term/${encodeURIComponent(studentId)}`)
      .then((response) => {
...
const handleSubmit = (e) => {
  ...
  const handleUpdateSubmit = () => {
    ...
    axios
    .put(`http://localhost:5000/api/update-first/${recordToEdit.id}`, updatedData)
    .then((response) => {
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Add First Term Examination</h2>
      {message && (
        <p
          style={{
            ...styles.message,
