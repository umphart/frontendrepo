...
const Senior3 = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/Senior3')
      .then(response => {
        setStudents(response.data);
        setLoading(false);
      })
      .catch(error => {
        console.error('Error fetching Senior 3 students:', error);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Senior 3 Class</h2>
      {loading ? (
