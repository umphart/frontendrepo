...
const navigate = useNavigate();
  useEffect(() => {
    axios.get('http://localhost:5000/api/Senior2')
      .then(response => {
        setStudents(response.data);
        setLoading(false);
      })
      .catch(error => {
        console.error('Error fetching Senior 2 students:', error);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Senior 2 Class</h2>
      {loading ? (
