...
const Senior1 = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/Senior1')
      .then(response => {
        setStudents(response.data);
        setLoading(false);
      })
      .catch(error => {
        console.error('Error fetching Senior 1 students:', error);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.heading}>Senior 1 Class</h2>
      {loading ? (
